  ...
  return (
    <header className="relative p-4 flex justify-between gap-4 items-center bg-white">
        ...
        <nav className={`flex flex-col md:flex-row md:gap-4 md:justify-center absolute md:static ${navShow? 'left-0': 'left-full' } top-full w-full bg-orange-100 md:bg-transparent z-50`}>
            ...
            {navLinks.map(link => <Link key={link.id} to={link.url} className='text-gray-600 text-sm sm:text-base py-2 px-4
             hover:bg-orange-500 hover:text-white md:border-b-2 md:border-transparent
              md:hover:border-orange-500 md:hover:bg-transparent
                md:hover:text-gray-600'>{link.text}</Link>)}
        </nav>

        <div onClick={() => setNavShow(!navShow)} className="flex p-2 border hover:bg-gray-200 md:hidden border-gray-200 rounded-md cursor-pointer">
